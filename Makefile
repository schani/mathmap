...
CFLAGS = -I. $(CGEN_CFLAGS) -Wall -O2 $(GLIB_CFLAGS) -DCMDLINE
LDFLAGS = $(LIBFFM) $(GLIB_LDFLAGS) -lm -ljpeg -lpng -lz
ifeq ($(MOVIES),YES)
...
CFLAGS = -I. $(CGEN_CFLAGS) -O2 -Wall $(GIMP_CFLAGS) -DGIMP -DLOCALEDIR=\"$(LOCALEDIR)\" $(NLS_CFLAGS)
...
ifneq ($(CMDLINE),YES)
	...
	if [ ! -f $(HOME)/$(GIMPDIR)/mathmap/new_template.c ] ; then cp new_template.c $(HOME)/$(GIMPDIR)/mathmap/ ; fi
endif
